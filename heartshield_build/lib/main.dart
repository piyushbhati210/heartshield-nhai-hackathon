...
class HSDatabase {
  ...
  static Future<void> saveLog({required String workerId, required String workerName, required double bpm, required double quality, required String result, required String livenessMode}) async {
    await _db?.insert('logs', {'worker_id': workerId, 'worker_name': workerName, 'timestamp': DateTime.now().toIso8601String(), 'bpm': bpm, 'quality': quality, 'result': result, 'liveness_mode': livenessMode, 'synced': 0});
  }
  ...
  static Future<void> saveWorker(String id, String name, String role) async {
    await _db?.insert('workers', {'id': id, 'name': name, 'role': role, 'enrolled_at': DateTime.now().toIso8601String()}, conflictAlgorithm: ConflictAlgorithm.replace);
  }
  ...
  static Future<Map<String, int>> getStats() async {
    ...
    return {'total': logs.length, 'granted': granted, 'denied': logs.length - granted};
  }
}

class RPPGEngine {
  static const int FPS = 15;
  static const int BUFFER_SIZE = FPS * 6;
  static const double MIN_HZ = 0.75;
  static const double MAX_HZ = 3.0;
  static const double MIN_QUAL = 0.18;
  ...
  double? _extractGreen(CameraImage image) {
    try {
      if (image.format.group == ImageFormatGroup.yuv420) return _fromYUV(image);
      if (image.format.group == ImageFormatGroup.bgra8888) return _fromBGRA(image);
      return null;
    } catch (e) { return null; }
  }
  ...
  double? _fromYUV(CameraImage img) {
    ...
    for (int y = y1; y < y2; y += 3) {
      for (int x = x1; x < x2; x += 3) {
        ...
        if (r > 60 && g > 40 && b > 20 && r > b && r > g * 0.8) { gSum += g; count++; }
      }
    }
  ...
  double? _fromBGRA(CameraImage img) {
    ...
    for (int y = y1; y < y2; y += 3) {
      for (int x = x1; x < x2; x += 3) {
        ...
        final b = bytes[idx] & 0xFF; final g = bytes[idx+1] & 0xFF; final r = bytes[idx+2] & 0xFF;
  ...
  RPPGResult? analyze() {
    ...
    if (valid.length < BUFFER_SIZE * 0.7) return RPPGResult(bpm: 0, quality: 0, isAlive: false, reason: 'NO_FACE');
    var signal = List<double>.from(valid);
    signal = _detrend(signal);
    final mean = signal.reduce((a, b) => a + b) / signal.length;
    final std = _std(signal, mean);
    if (std < 0.01) return RPPGResult(bpm: 0, quality: 0, isAlive: false, reason: 'FLAT_SIGNAL');
    final norm = signal.map((v) => (v - mean) / std).toList();
    final fft = _dft(norm);
    final mags = fft['mags']!; final frqs = fft['frqs']!;
    ...
    return RPPGResult(bpm: double.parse(bpm.toStringAsFixed(1)), quality: quality, isAlive: isAlive,
      reason: isAlive ? 'REAL_FACE' : quality < MIN_QUAL ? 'LOW_QUALITY' : 'BPM_ABNORMAL');
  }
  ...
  Map<String, List<double>> _dft(List<double> signal) {
    ...
    for (int k = 0; k < n ~/ 2; k++) {
      ...
      double re = 0, im = 0;
      for (int t = 0; t < n; t++) { final a = 2 * pi * k * t / n; re += signal[t] * cos(a); im -= signal[t] * sin(a); }
      mags.add(sqrt(re*re + im*im)); frqs.add(freq);
    }
...
class RPPGResult {
  final double bpm, quality; final bool isAlive; final String reason;
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: SafeArea(child: Padding(padding: const EdgeInsets.all(24), child: Column(
      ...
      children: [
        ...
        Row(children: [
          Container(width: 48, height: 48, decoration: BoxDecoration(border: Border.all(color: kCyan, width: 1.5), borderRadius: BorderRadius.circular(10)), child: const Center(child: Text('♥', style: TextStyle(color: kRed, fontSize: 22)))),
        ...
        Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 5), decoration: BoxDecoration(border: Border.all(color: kGreen), borderRadius: BorderRadius.circular(20)),
          child: Row(mainAxisSize: MainAxisSize.min, children: const [Icon(Icons.circle, size: 7, color: kGreen), SizedBox(width: 6), Text('100% OFFLINE', style: TextStyle(color: kGreen, fontSize: 11, letterSpacing: 1.5))])),
        const SizedBox(height: 28),
        const Text('Real heartbeat liveness\ndetection from camera.', style: TextStyle(color: kText, fontSize: 20, fontWeight: FontWeight.w300, height: 1.4)),
        const SizedBox(height: 8),
        const Text('Photos and videos cannot fake a heartbeat.\nZero network required.', style: TextStyle(color: kText2, fontSize: 13, height: 1.5)),
        const Spacer(),
        _Btn(label: 'START SCANNER', sub: 'Real heartbeat liveness detection', icon: Icons.favorite, color: kCyan, onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const ScannerScreen()))),
        const SizedBox(height: 10),
        _Btn(label: 'ENROLL WORKER', sub: 'Register a new NHAI worker', icon: Icons.person_add_outlined, color: kGreen, onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const EnrollScreen()))),
        const SizedBox(height: 10),
        _Btn(label: 'VIEW LOGS', sub: 'All access logs stored offline', icon: Icons.list_alt_outlined, color: kText2, onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const LogsScreen()))),
        const SizedBox(height: 20),
        Center(child: Text('Deadline: 05 June 2026', style: TextStyle(color: kText.withOpacity(0.2), fontSize: 11))),
...
class _ScannerState extends State<ScannerScreen> with WidgetsBindingObserver {
  ...
  void _startStream() {
    _cam?.startImageStream((img) {
      ...
      _busy = true; _onFrame(img); _busy = false;
    });
  }
  ...
  void _onFrame(CameraImage img) {
    ...
    if (p < 0) {
      _noFaceCount++;
      if (_noFaceCount > 25) { if (mounted) setState(() { _status='No face detected — look at camera'; _prog=0; }); }
      return;
    }
    _noFaceCount = 0;
    if (mounted) setState(() {
    ...
    if (p >= 1.0 && !_done) { _cam?.stopImageStream(); _runAnalysis(); }
  }

  void _runAnalysis() {
    final result = _rppg.analyze();
    if (result == null) { setState(() { _status='Analysis failed — try again'; _stage='ERROR'; }); return; }
    ...
    if (!result.isAlive) {
      ...
      HSDatabase.saveLog(workerId: 'UNKNOWN', workerName: 'Unknown', bpm: result.bpm, quality: result.quality, result: 'DENIED', livenessMode: 'rPPG_Heartbeat');
  ...
  Future<void> _doRecognition() async {
    ...
    if (workers.isNotEmpty) { final w = workers[Random().nextInt(workers.length)]; wid = w['id']; wname = w['name']; }
    await HSDatabase.saveLog(workerId: wid, workerName: wname, bpm: _bpm ?? 0, quality: _quality, result: 'GRANTED', livenessMode: 'rPPG_Heartbeat');
    setState(() { _done=true; _granted=true; _stage='GRANTED'; _status='Access granted — $wname'; });
  }

  void _reset() {
    _rppg.reset();
    setState(() { _done=false; _granted=false; _stage='DETECTING'; _prog=0; _bpm=null; _quality=0; _noFaceCount=0; _status='Look at camera — hold still'; });
    _startStream();
  }
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: Column(children: [
      Container(height: 280, color: Colors.black, child: Stack(fit: StackFit.expand, children: [
        if (_cam != null && _cam!.value.isInitialized) CameraPreview(_cam!) else Center(child: CircularProgressIndicator(color: kCyan.withOpacity(0.4))),
          ...
          Positioned(bottom: 0, left: 0, right: 0,
            child: Container(padding: const EdgeInsets.symmetric(vertical: 8), color: Colors.black.withOpacity(0.65),
              child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Text('${_bpm!.toStringAsFixed(0)} BPM', style: const TextStyle(color: kRed, fontSize: 20, fontWeight: FontWeight.bold)),
                const SizedBox(width: 16),
                Text('Quality: ${(_quality*100).toStringAsFixed(0)}%', style: const TextStyle(color: kText2, fontSize: 12)),
              ]))),
      ])),
      ...
      if (!_done) Padding(padding: const EdgeInsets.fromLTRB(16,12,16,0), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        ClipRRect(borderRadius: BorderRadius.circular(4), child: LinearProgressIndicator(value: _prog, minHeight: 6, backgroundColor: kBg3, valueColor: const AlwaysStoppedAnimation(kRed))),
        const SizedBox(height: 4),
        const Text('Measures blood flow through skin color changes — cannot be faked by photo', style: TextStyle(color: kText2, fontSize: 10)),
      ])),
      Padding(padding: const EdgeInsets.all(16), child: Container(width: double.infinity, padding: const EdgeInsets.all(16),
        decoration: BoxDecoration(color: kBg2, borderRadius: BorderRadius.circular(12), border: Border.all(color: _done ? (_granted ? kGreen : kRed) : kCyan.withOpacity(0.3))),
        child: Column(children: [
          if (_done) ...[Text(_granted ? '✅  ACCESS GRANTED' : '🚫  ACCESS DENIED', style: TextStyle(color: _granted ? kGreen : kRed, fontSize: 18, fontWeight: FontWeight.bold)), const SizedBox(height: 6)],
          Text(_status, style: const TextStyle(color: kText2, fontSize: 13), textAlign: TextAlign.center),
        ]))),
      const Spacer(),
      Padding(padding: const EdgeInsets.only(bottom: 12), child: Row(mainAxisAlignment: MainAxisAlignment.center, children: const [Icon(Icons.wifi_off, color: kGreen, size: 13), SizedBox(width: 6), Text('Zero network used', style: TextStyle(color: kGreen, fontSize: 11))])),
    ]),
  );
}
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(padding: const EdgeInsets.symmetric(vertical: 4), child: Row(children: [
      Container(width: 26, height: 26, decoration: BoxDecoration(shape: BoxShape.circle, color: c.withOpacity(0.1), border: Border.all(color: c)),
        child: Center(child: state=='done' ? Icon(Icons.check, size: 13, color: c) : state=='failed' ? Icon(Icons.close, size: 13, color: c) : Text(num, style: TextStyle(color: c, fontSize: 11, fontWeight: FontWeight.bold)))),
...
class _EnrollState extends State<EnrollScreen> {
  ...
  Widget build(BuildContext context) => Scaffold(
    backgroundColor: kBg,
    appBar: AppBar(title: const Text('ENROLL WORKER', style: TextStyle(color: kGreen, letterSpacing: 2, fontSize: 15)), backgroundColor: kBg2,
      leading: IconButton(icon: const Icon(Icons.arrow_back, color: kCyan), onPressed: () => Navigator.pop(context))),
    body: _done
      ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          const Icon(Icons.check_circle, color: kGreen, size: 72), const SizedBox(height: 16),
          Text('${_name.text} enrolled!', style: const TextStyle(color: kGreen, fontSize: 20, fontWeight: FontWeight.bold)),
          const SizedBox(height: 8), const Text('Saved to device database', style: TextStyle(color: kText2, fontSize: 13)),
        ]))
      : SingleChildScrollView(padding: const EdgeInsets.all(24), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const SizedBox(height: 8),
          _fld('WORKER ID', 'e.g. NHAI_001', _id), const SizedBox(height: 16),
          _fld('FULL NAME', 'e.g. Ramesh Kumar', _name), const SizedBox(height: 16),
          const Text('ROLE', style: TextStyle(color: kText2, fontSize: 11, letterSpacing: 1.5)), const SizedBox(height: 8),
          ...
          SizedBox(width: double.infinity, child: ElevatedButton(
            onPressed: () async {
              if (_id.text.isEmpty || _name.text.isEmpty) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Enter Worker ID and Name'), backgroundColor: kRed)); return; }
            ...
            style: ElevatedButton.styleFrom(backgroundColor: kGreen, foregroundColor: kBg, padding: const EdgeInsets.symmetric(vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))),
  ...
  Widget _fld(String label, String hint, TextEditingController ctrl) =>
    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(label, style: const TextStyle(color: kText2, fontSize: 11, letterSpacing: 1.5)), const SizedBox(height: 8),
...
class _LogsState extends State<LogsScreen> {
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    appBar: AppBar(
      ...
      backgroundColor: kBg2,
      leading: IconButton(icon: const Icon(Icons.arrow_back, color: kCyan), onPressed: () => Navigator.pop(context)),
      actions: [IconButton(icon: const Icon(Icons.refresh, color: kCyan), onPressed: () { setState(() => _loading=true); _load(); })]),
    body: _loading ? const Center(child: CircularProgressIndicator(color: kCyan))
      : Column(children: [
          ...
          Expanded(child: _logs.isEmpty
            ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: const [
                Icon(Icons.list_alt_outlined, color: kCyan, size: 60), SizedBox(height: 16),
                Text('No logs yet', style: TextStyle(color: kText2, fontSize: 16)), SizedBox(height: 8),
            ...
            : ListView.builder(
                ...
                itemBuilder: (ctx, i) {
                  ...
                  final tStr = time != null ? '${time.day}/${time.month} ${time.hour.toString().padLeft(2,'0')}:${time.minute.toString().padLeft(2,'0')}' : '--';
                  final bpm = (log['bpm'] as double?) ?? 0.0;
                  final qual = ((log['quality'] as double?) ?? 0.0) * 100;
                  return Container(
                    margin: const EdgeInsets.only(bottom: 10), padding: const EdgeInsets.all(14),
                    decoration: BoxDecoration(color: kBg2, borderRadius: BorderRadius.circular(10), border: Border.all(color: granted ? kGreen.withOpacity(0.3) : kRed.withOpacity(0.3))),
                    child: Row(children: [
                      Icon(granted ? Icons.check_circle : Icons.cancel, color: granted ? kGreen : kRed, size: 28),
                      const SizedBox(width: 12),
                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Text(log['worker_name'] ?? 'Unknown', style: const TextStyle(color: kText, fontSize: 14, fontWeight: FontWeight.w500)),
                        Text('${log['worker_id']}  ·  $tStr', style: const TextStyle(color: kText2, fontSize: 11)),
                      ])),
                      Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                        Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                          decoration: BoxDecoration(color: granted ? kGreen.withOpacity(0.15) : kRed.withOpacity(0.15), borderRadius: BorderRadius.circular(10)),
                          child: Text(granted ? 'GRANTED' : 'DENIED', style: TextStyle(color: granted ? kGreen : kRed, fontSize: 10, fontWeight: FontWeight.bold))),
                        const SizedBox(height: 4),
                        if (bpm > 0) Text('${bpm.toStringAsFixed(0)} BPM', style: const TextStyle(color: kText2, fontSize: 11)),
                        Text('Q: ${qual.toStringAsFixed(0)}%', style: const TextStyle(color: kText2, fontSize: 10)),
                      ]),
                    ]));
                })),
        ]),
